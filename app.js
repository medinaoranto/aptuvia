...
// Cabecera de marca de los PDF de exámenes y notas: nuestro logo pequeño a la
// izquierda, sin protagonismo, y a la derecha el nombre de la academia con el
// mismo peso visual. En Aula Abierta no hay academia detrás: la derecha va vacía
// y el profesor aparece más abajo, en los datos del examen.
...
async function descargarMateriaZip(){
  ...
  try{
    ...
    examenes=(examenes||[]).filter(ex=>!/^repaso-/.test(String(ex.id)));
...
async function loadData(){
  ...
  try{
    ...
    // Datos de marca para las cabeceras de los PDF
    window._brandAcademia=''; window._brandProfesor='';
    try{
      const ac=await call('/rest/v1/academia?select=nombre&limit=1');
      if(ac&&ac[0]&&ac[0].nombre) window._brandAcademia=ac[0].nombre;
    }catch(e){}
    try{
      if(isStaff()||window._saImpersona){
        const pid=window._saImpersonaProf||userId;
        if(pid===userId && userName){ window._brandProfesor=userName; }
        else{ const pf=await call('/rest/v1/perfiles?select=nombre&id=eq.'+encodeURIComponent(pid)); if(pf&&pf[0]&&pf[0].nombre) window._brandProfesor=pf[0].nombre; }
      }else if(userProfesorId){
        const pf=await call('/rest/v1/perfiles?select=nombre&id=eq.'+encodeURIComponent(userProfesorId));
        if(pf&&pf[0]&&pf[0].nombre) window._brandProfesor=pf[0].nombre;
      }
    }catch(e){}
...
function avPanel(area,vis){
  ...
  if(!vis.length){
    ...
    vis.forEach(a=>{
      ...
      const mNum=(a.texto||'').match(/P-\d{4}-\d{4}/);
      const ojo=mNum?`<button onclick="avVerPresu('${mNum[0]}')" title="Ver el presupuesto" style="flex:0 0 auto;background:#eef4fb;border:1px solid #c7d2fe;border-radius:6px;width:22px;height:22px;line-height:1;font-size:.8rem;cursor:pointer;padding:0">👁</button>`:'';
...
function avToggle(){ window._avAbierta=!window._avAbierta; if(window._avAbierta) window._aiAbierta=false; avPintarBarra(); aiPintarBarra(); avRowSync(); }
...
function aiToggle(){ window._aiAbierta=!window._aiAbierta; if(window._aiAbierta) window._avAbierta=false; avPintarBarra(); aiPintarBarra(); avRowSync(); }
function avRowSync(){ const r=document.getElementById('av-row'); if(r) r.classList.toggle('av-open', !!(window._avAbierta||window._aiAbierta)); }
async function avVerPresu(num){
  try{
    const rows=await call('/rest/v1/presupuestos?select=*&numero=eq.'+encodeURIComponent(num));
    const p=(rows||[])[0];
    if(!p){ appAlert('No encuentro el presupuesto '+num+'.'); return; }
    const ov=document.createElement('div');
    ...
    const box=document.createElement('div');
    box.style.cssText='background:#fff;border-radius:16px;max-width:560px;width:100%;padding:16px 18px;box-shadow:0 20px 50px rgba(0,0,0,.3)';
    box.innerHTML='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px"><b style="color:var(--navy)">👁 Presupuesto '+escHtml(num)+'</b><button id="avp-x" style="background:#fff;border:1.5px solid var(--line);border-radius:8px;padding:4px 10px;cursor:pointer">✕</button></div>'+presuFichaHTML(p)+'<p style="font-size:.72rem;color:var(--ink-soft);margin:12px 2px 0">Para dar de alta al cliente: Academias y profesores → Crear usuario.</p>';
    ov.appendChild(box); document.body.appendChild(ov);
    document.getElementById('avp-x').onclick=()=>ov.remove();
...
async function saCrearAcademiaUI(){
  ...
  try{ await call('/rest/v1/rpc/sa_crear_academia',{method:'POST',body:{p_nombre:n}}); await avisarAdmin('🏫 Academia creada SIN presupuesto: "'+n.trim()+'". NO facturar por defecto (alta interna/cortesía). Si es cliente de pago, crea su presupuesto y factura desde ahí (0 € si es cortesía).'); await openSuperadmin('Academia creada.'); }
  catch(e){ appAlert('No se pudo: '+(e.message||'')); }
}
...
function saRenderFacturacionLista(){
  ...
  if(!sub){
    ...
    h+=`<button class="fact-menu" onclick="saFactSub('archivo')" style="grid-column:1/-1"><b>🗄 Archivo central</b><span>Ficha de todos los clientes (desde presupuestos) y archivo de contratos y documentación de la empresa</span></button>`;
    h+=`</div>`;
    h+=docsBar('admin');
    $('teacher').innerHTML=saShell(h);
    return;
  ...
  if(sub==='archivo'){ arRender(); return; }
...
async function arClientes(){
  ...
  lista.forEach(p=>{
    ...
    if(!f.provincia && c.provincia) f.provincia=c.provincia;
...
async function arDocs(){
  ...
  h+=`<div class="t-card">
    ...
    <input id="ad-tit" type="text" placeholder="Ej.: Contrato de licencia · Academia X">
    <label>Tipo</label>
    <select id="ad-tipo">${AR_TIPOS.map(t=>`<option value="${escAttr(t)}">${escHtml(t)}</option>`).join('')}</select>
    <label>Notas (opcional)</label>
    <textarea id="ad-notas" rows="2" placeholder="Referencia, fecha de firma, partes…" style="width:100%;box-sizing:border-box;font-family:inherit;font-size:.85rem;padding:8px 10px;border:1.5px solid var(--line);border-radius:10px;resize:vertical"></textarea>
    <label style="margin-top:10px">PDF del documento</label>
  ...
  if(!list.length){ h+=`<p class="sa-empty">Aún no hay documentos.</p>`; }
    ...
    h+=`<div style="border:1.5px solid var(--line);border-radius:12px;padding:11px 13px;margin-bottom:9px;background:#fff">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px">
        <div style="min-width:0"><b style="font-size:.9rem;color:var(--navy)">${escHtml(d.titulo||'—')}</b>
...
async function arDocGuardar(){
  ...
  const tipo=$('ad-tipo')&&$('ad-tipo').value||'Documento';
  const notas=($('ad-notas')&&$('ad-notas').value||'').trim();
  const file=$('ad-file')&&$('ad-file').files&&$('ad-file').files[0];
  if(!tit){ appAlert('Ponle un título al documento.'); return; }
  if(!file){ appAlert('Adjunta el PDF del documento.'); return; }
  const b=$('ad-guardar'); if(b){ b.disabled=true; b.innerHTML='<span class="spin"></span>'; }
  try{
    const safe=(file.name||'documento.pdf').replace(/[^a-zA-Z0-9._-]/g,'_').slice(-60);
    const path='docs/'+Date.now()+'_'+safe;
    const up=await fetch(SUPABASE_URL+'/storage/v1/object/documentos-empresa/'+encodeURI(path),{method:'POST',headers:{'apikey':SUPABASE_KEY,'Authorization':'Bearer '+token,'Content-Type':file.type||'application/pdf'},body:file});
    if(!up.ok){ let det=''; try{ const t=await up.text(); const j=JSON.parse(t); det=j.message||j.error||t; }catch(e){} throw new Error('Storage '+up.status+': '+(det||'sin detalle')); }
    await call('/rest/v1/documentos_empresa',{method:'POST',body:{titulo:tit,tipo:tipo,notas:notas||null,archivo_url:path,archivo_nombre:file.name}});
    arDocs();
  }catch(e){ if(b){ b.disabled=false; b.textContent='Guardar documento'; } appAlert('No se pudo guardar: '+(e.message||'')); }
}
async function arDocSigned(path){
  const r=await fetch(SUPABASE_URL+'/storage/v1/object/sign/documentos-empresa/'+encodeURI(path),{method:'POST',headers:{apikey:SUPABASE_KEY,Authorization:'Bearer '+token,'Content-Type':'application/json'},body:JSON.stringify({expiresIn:3600})});
  const d=await r.json(); if(!d||!d.signedURL) throw new Error('sin URL'); return SUPABASE_URL+'/storage/v1'+d.signedURL;
}
async function arDocVer(path){ try{ window.open(await arDocSigned(path),'_blank'); }catch(e){ appAlert('No se pudo abrir el PDF.'); } }
async function arDocDescargar(path,nombre){ try{ const u=await arDocSigned(path); window.open(u+'&download='+encodeURIComponent(nombre||'documento.pdf'),'_blank'); }catch(e){ appAlert('No se pudo descargar.'); } }
async function arDocDrive(path,nombre){
  try{
    const u=await arDocSigned(path);
    try{
      const resp=await fetch(u); const blob=await resp.blob();
      const file=new File([blob], nombre||'documento.pdf', {type:blob.type||'application/pdf'});
      if(navigator.canShare && navigator.canShare({files:[file]})){ await navigator.share({files:[file], title:nombre||'Documento'}); return; }
    }catch(_){}
    const a=document.createElement('a'); a.href=u+'&download='+encodeURIComponent(nombre||'documento.pdf'); a.click();
    driveAbrir();
    appAlert('Descargado. Se ha abierto Drive: súbelo ahí desde tus descargas.');
  }catch(e){ appAlert('No se pudo compartir a Drive.'); }
}
async function arDocBorrar(id,path){
  if(!await appConfirm('¿Borrar este documento y su PDF? No se puede deshacer.')) return;
  try{
    if(path){ try{ await fetch(SUPABASE_URL+'/storage/v1/object/documentos-empresa/'+encodeURI(path),{method:'DELETE',headers:{'apikey':SUPABASE_KEY,'Authorization':'Bearer '+token}}); }catch(_){} }
    await call('/rest/v1/documentos_empresa?id=eq.'+id,{method:'DELETE'});
    arDocs();
  }catch(e){ appAlert('No se pudo borrar: '+(e.message||'')); }
}
...
async function pxAcepSubir(id,input){
  const file=input&&input.files&&input.files[0]; if(!file) return;
  const p=pxLista.find(x=>String(x.id)===String(id));
  try{
    const safe=(file.name||'aceptacion.pdf').replace(/[^a-zA-Z0-9._-]/g,'_').slice(-60);
    const path='aceptaciones/'+id+'_'+Date.now()+'_'+safe;
    const up=await fetch(SUPABASE_URL+'/storage/v1/object/documentos-empresa/'+encodeURI(path),{method:'POST',headers:{'apikey':SUPABASE_KEY,'Authorization':'Bearer '+token,'Content-Type':file.type||'application/pdf'},body:file});
    if(!up.ok){ let det=''; try{ const t=await up.text(); const j=JSON.parse(t); det=j.message||j.error||t; }catch(e){} throw new Error('Storage '+up.status+': '+(det||'sin detalle')); }
    await call('/rest/v1/presupuestos?id=eq.'+id,{method:'PATCH',body:{aceptacion_url:path,aceptacion_nombre:file.name}});
    if(p){ p.aceptacion_url=path; p.aceptacion_nombre=file.name; }
    pxPintar();
  }catch(e){ appAlert('No se pudo adjuntar: '+(e.message||'')); }
}
async function pxAcepVer(id){ const p=pxLista.find(x=>String(x.id)===String(id)); if(!p||!p.aceptacion_url) return; try{ window.open(await arDocSigned(p.aceptacion_url),'_blank'); }catch(e){ appAlert('No se pudo abrir.'); } }
function pxAcepDrive(id){ const p=pxLista.find(x=>String(x.id)===String(id)); if(!p||!p.aceptacion_url) return; arDocDrive(p.aceptacion_url, p.aceptacion_nombre||'aceptacion.pdf'); }
async function pxAcepBorrar(id){
  const p=pxLista.find(x=>String(x.id)===String(id)); if(!p||!p.aceptacion_url) return;
  if(!await appConfirm('¿Quitar el PDF de aceptación?')) return;
  const path=p.aceptacion_url;
  try{
    try{ await fetch(SUPABASE_URL+'/storage/v1/object/documentos-empresa/'+encodeURI(path),{method:'DELETE',headers:{'apikey':SUPABASE_KEY,'Authorization':'Bearer '+token}}); }catch(_){}
    await call('/rest/v1/presupuestos?id=eq.'+id,{method:'PATCH',body:{aceptacion_url:null,aceptacion_nombre:null}});
    p.aceptacion_url=null; p.aceptacion_nombre=null;
    pxPintar();
  }catch(e){ appAlert('No se pudo quitar: '+(e.message||'')); }
}
...
function factMostrarPlantilla(){
  ...
  box.innerHTML=`<div style="padding:16px 18px 8px;font-weight:800;color:var(--navy)">✉️ Enviar factura al cliente</div>
    <div style="padding:0 18px;overflow-y:auto">
      ...
      <p style="font-size:.72rem;color:var(--ink-soft);margin:4px 0 10px">Se abrirá tu app de correo con el mensaje ya redactado; comprueba que el remitente sea <b>contacto@aptuvia.es</b> y adjunta el PDF descargado antes de enviar.</p>
    </div>
  ...
  document.getElementById('fm-ok').onclick=async ()=>{
    ...
    ov.remove();
    if(!await appConfirm('¿Abrir tu correo para enviar a '+to+'?')) return;
    ...
    const link='mailto:'+to+'?subject='+encodeURIComponent(sub)+'&body='+encodeURIComponent(body);
...
function pintarTeacher(){
  ...
  $('teacher').innerHTML=`${window._saImpersona?`<button class="backbtn" onclick="salirImpersona()" style="margin-bottom:10px;background:var(--honey-tint);border-color:var(--honey)">← Volver a la academia</button>`:''}<div class="t-welcome" style="display:flex;align-items:center;justify-content:space-between;gap:10px">
      <div class="t-course-pill">${cabecera}</div>
      <div style="display:flex;align-items:center;gap:9px;flex-shrink:0">
      <button onclick="openCalendario('prof')" title="Calendario" style="background:none;border:none;cursor:pointer;font-size:1.5rem;line-height:1;padding:2px 0">📅</button>
      <button id="prof-bell" onclick="openAvisosProfe()" title="Mis avisos" style="background:none;border:none;cursor:pointer;font-size:1.5rem;position:relative;line-height:1;padding:2px 0">🔔<span id="prof-bell-dot" style="display:none;position:absolute;top:-3px;right:-5px;min-width:16px;height:16px;padding:0 3px;background:#e11d1d;color:#fff;border-radius:9px;font-size:.62rem;font-weight:800;line-height:16px;text-align:center;border:1.5px solid #fff"></span></button>
      </div>
    </div>
    <div class="t-grid">
      ...
      <button class="t-tile" onclick="openExamMgmt()">
        <span class="ic" style="background:var(--honey-tint)">⚙️</span><span class="tt">Crear y gestionar exámenes</span><span class="ts">Test, redacción y banco</span></button>
  ...
  if(!window._demoMode){ Promise.all([
    ...
    call('/rest/v1/rpc/reportes_prof_no_leidos',{method:'POST',body:{}}).catch(()=>0)
  ]).then(function(r){ const n=(+r[0]||0)+(+r[1]||0)+(+r[2]||0); const el=$('t-soporte-badge'); if(el && n>0){ el.className='tile-badge'; el.textContent=String(n); } }); }
  if(!window._demoMode){ Promise.all([
  ...
  ]).then(function(r){ const n=((r[0]||[]).filter(avProfDue).length)+((r[1]||[]).filter(e=>!e.hecho).length); const dot=$('prof-bell-dot'); if(dot && n>0){ dot.style.display='block'; dot.textContent=String(n); } }); }
}
...
// ---- Cambiar contraseña ----
// ===== Avisos privados del profesor (campanita) =====
const AV_DOW=['Lunes','Martes','Miércoles','Jueves','Viernes','Sábado','Domingo'];
let _avProfDraft={texto:'',repetir:'no',fecha:'',dia_semana:1,dia_mes:1};
function avProfIsoHoy(){ const j=new Date().getDay(); return j===0?7:j; }
function avProfDue(m){
  if(m.activo===false) return false;
  if(m.repetir==='semanal') return avProfIsoHoy()===(m.dia_semana||1);
  if(m.repetir==='mensual') return new Date().getDate()===(m.dia_mes||1);
  if(m.fecha) return String(m.fecha)<=fechaISOLocal(new Date());
  return false;
}
function avProfCad(m){
  if(m.repetir==='semanal') return 'Cada '+AV_DOW[(m.dia_semana||1)-1];
  if(m.repetir==='mensual') return 'Día '+(m.dia_mes||1)+' de cada mes';
  return 'Puntual · '+(m.fecha||'sin fecha');
}
function avProfSetRep(v){ const t=$('avp-txt'); if(t) _avProfDraft.texto=t.value; _avProfDraft.repetir=v; avProfRender(window._avProfLista||[], null, !!window._demoMode); }
function avProfRender(lista, okMsg, demo){
  window._avProfLista=lista;
  const d=_avProfDraft;
  const hoy=(lista||[]).filter(avProfDue);
  const h=['<button class="backbtn" onclick="pintarTeacher()">← Panel</button>'];
  h.push('<h1 style="font-size:1.25rem;font-weight:800;letter-spacing:-.4px;margin:6px 0 2px;color:var(--navy)">🔔 Mis avisos</h1>');
  h.push('<p style="font-size:.8rem;color:var(--ink-soft);margin-bottom:12px">Recordatorios privados para ti. Solo los ves tú.</p>');
  if(okMsg) h.push('<div class="t-note ok">'+escHtml(okMsg)+'</div>');
  const calHoy=(demo?[]:(window._calHoyProf||[]));
  if(hoy.length || calHoy.length){
    h.push('<div class="t-card" style="border-color:var(--honey);background:var(--honey-tint)"><b style="font-size:.8rem;color:var(--honey-deep)">📌 Para hoy</b>');
    hoy.forEach(m=>h.push('<div style="font-size:.88rem;color:var(--navy);margin-top:6px">• '+escHtml(m.texto)+'</div>'));
    calHoy.forEach(e=>h.push('<div style="font-size:.88rem;color:var(--navy);margin-top:6px">📅 '+escHtml(e.titulo)+(e.nota?(' — '+escHtml(e.nota)):'')+'</div>'));
    h.push('</div>');
  }
  h.push('<div class="t-card">');
  h.push('<label style="margin-top:2px">Nuevo aviso</label>');
  h.push('<textarea id="avp-txt" rows="2" placeholder="Ej.: Corregir redacciones del viernes"'+(demo?' disabled':'')+'>'+escHtml(d.texto||'')+'</textarea>');
  h.push('<label style="margin-top:8px">Cuándo</label>');
  h.push('<select id="avp-rep"'+(demo?' disabled':'')+' onchange="avProfSetRep(this.value)"><option value="no"'+(d.repetir==='no'?' selected':'')+'>Puntual (una fecha)</option><option value="semanal"'+(d.repetir==='semanal'?' selected':'')+'>Cada semana</option><option value="mensual"'+(d.repetir==='mensual'?' selected':'')+'>Cada mes</option></select>');
  if(d.repetir==='no') h.push('<input type="date" id="avp-fecha" value="'+escAttr(d.fecha||'')+'" style="margin-top:8px;width:100%;padding:9px;border:1px solid var(--line);border-radius:10px;font-size:.9rem">');
  else if(d.repetir==='semanal') h.push('<select id="avp-dsem" style="margin-top:8px">'+AV_DOW.map((n,i)=>'<option value="'+(i+1)+'"'+((d.dia_semana||1)===i+1?' selected':'')+'>'+n+'</option>').join('')+'</select>');
  else h.push('<input type="number" id="avp-dmes" min="1" max="31" value="'+(d.dia_mes||1)+'" style="margin-top:8px;width:100%;padding:9px;border:1px solid var(--line);border-radius:10px;font-size:.9rem">');
  h.push('<button class="btn btn-honey" id="avp-add" style="margin-top:12px"'+(demo?' disabled':'')+'>Añadir aviso</button>');
  h.push('</div>');
  h.push('<h2 style="font-size:.78rem;font-weight:700;color:var(--ink-soft);text-transform:uppercase;letter-spacing:1px;margin:18px 2px 10px">Todos mis avisos</h2>');
  if(!(lista||[]).length) h.push('<p style="font-size:.85rem;color:var(--ink-soft);text-align:center;margin:8px 0">No tienes avisos todavía.</p>');
  else lista.forEach(m=>{
    h.push('<div class="t-card" style="padding:11px 13px;'+(m.activo===false?'opacity:.6':'')+'">');
    h.push('<div style="font-size:.9rem;color:var(--navy);white-space:pre-wrap">'+escHtml(m.texto)+'</div>');
    h.push('<div style="font-size:.72rem;color:var(--ink-soft);margin-top:3px">'+avProfCad(m)+'</div>');
    h.push('<div style="display:flex;gap:8px;margin-top:8px">');
    h.push('<button onclick="avProfActivar(\''+m.id+'\','+(m.activo===false?'true':'false')+')"'+(demo?' disabled':'')+' style="font-size:.72rem;padding:5px 11px;border-radius:14px;cursor:pointer;border:1.5px solid '+(m.activo===false?'#cbd5e1':'#15803d')+';background:'+(m.activo===false?'#fff':'#dcfce7')+';color:'+(m.activo===false?'var(--ink-soft)':'#15803d')+';font-weight:700">'+(m.activo===false?'○ Pausado':'● Activo')+'</button>');
    h.push('<button onclick="avProfBorrar(\''+m.id+'\')"'+(demo?' disabled':'')+' style="font-size:.72rem;padding:5px 11px;border-radius:14px;cursor:pointer;border:1.5px solid #f3c4c4;background:#fdeaea;margin-left:auto">🗑</button>');
    h.push('</div></div>');
  });
  if(demo) h.push('<p style="font-size:.72rem;color:var(--ink-soft);text-align:center;margin-top:10px">En la demo los avisos son de muestra.</p>');
  $('teacher').innerHTML=h.join('');
  const add=$('avp-add'); if(add) add.onclick=avProfCrear;
}
async function openAvisosProfe(okMsg){
  showView('teacher'); window.scrollTo(0,0);
  if(window._demoMode){
    avProfRender([
      {id:'d1',texto:'Corregir redacciones del viernes',repetir:'semanal',dia_semana:5,activo:true},
      {id:'d2',texto:'Abrir el Tema 3 a la clase',repetir:'no',fecha:fechaISOLocal(new Date()),activo:true},
      {id:'d3',texto:'Repasar notas a fin de mes',repetir:'mensual',dia_mes:28,activo:false}
    ], okMsg, true);
    return;
  }
  $('teacher').innerHTML='<button class="backbtn" onclick="pintarTeacher()">← Panel</button><div class="loader"><span class="spin"></span></div>';
  let lista=[];
  try{ lista=await call('/rest/v1/rpc/prof_avisos_listar',{method:'POST',body:{}})||[]; }
  catch(e){ $('teacher').innerHTML='<button class="backbtn" onclick="pintarTeacher()">← Panel</button><div class="t-note err">No se pudieron cargar los avisos: '+escHtml(e.message||'')+'</div>'; return; }
  try{ const hy=fechaISOLocal(new Date()); window._calHoyProf=(await call('/rest/v1/rpc/cal_prof_listar',{method:'POST',body:{p_desde:hy,p_hasta:hy}})||[]).filter(e=>!e.hecho); }catch(e){ window._calHoyProf=[]; }
  avProfRender(lista, okMsg, false);
}
async function avProfCrear(){
  const t=$('avp-txt'); const txt=(t&&t.value||'').trim();
  if(!txt){ appAlert('Escribe el aviso.'); return; }
  const rep=$('avp-rep')?$('avp-rep').value:'no';
  const body={p_texto:txt, p_repetir:rep, p_fecha:null, p_dia_semana:null, p_dia_mes:null};
  if(rep==='no') body.p_fecha=($('avp-fecha')&&$('avp-fecha').value)||null;
  else if(rep==='semanal') body.p_dia_semana=+(($('avp-dsem')&&$('avp-dsem').value)||1);
  else if(rep==='mensual'){ let dm=+(($('avp-dmes')&&$('avp-dmes').value)||1); body.p_dia_mes=Math.max(1,Math.min(31,dm)); }
  const b=$('avp-add'); if(b){ b.disabled=true; b.innerHTML='<span class="spin"></span>'; }
  try{ await call('/rest/v1/rpc/prof_aviso_crear',{method:'POST',body}); _avProfDraft={texto:'',repetir:'no',fecha:'',dia_semana:1,dia_mes:1}; await openAvisosProfe('✅ Aviso añadido.'); }
  catch(e){ if(b){ b.disabled=false; b.textContent='Añadir aviso'; } appAlert('No se pudo: '+(e.message||'')); }
}
async function avProfActivar(id, activar){
  try{ await call('/rest/v1/rpc/prof_aviso_activar',{method:'POST',body:{p_id:id, p_activo:activar}}); await openAvisosProfe(); }
  catch(e){ appAlert('No se pudo: '+(e.message||'')); }
}
async function avProfBorrar(id){
  if(!await appConfirm('¿Borrar este aviso? No se puede deshacer.')) return;
  try{ await call('/rest/v1/rpc/prof_aviso_borrar',{method:'POST',body:{p_id:id}}); await openAvisosProfe('Aviso borrado.'); }
  catch(e){ appAlert('No se pudo: '+(e.message||'')); }
}
...
function calFechaLarga(iso){ try{ const p=iso.split('-'); return new Date(+p[0],+p[1]-1,+p[2]).toLocaleDateString('es-ES',{weekday:'long',day:'numeric',month:'long'}); }catch(e){ return iso; } }
function calFechaCorta(iso){ try{ const p=iso.split('-'); return p[2]+'/'+p[1]; }catch(e){ return iso; } }
function calCont(){ return window._cal.mode==='alum'?'home':'teacher'; }
function calBtnHtml(mode){ return '<button onclick="openCalendario(\''+mode+'\')" title="Calendario" style="background:none;border:1px solid var(--honey);border-radius:999px;padding:3px 10px;font-size:.68rem;color:var(--honey-deep);cursor:pointer;font-family:inherit;font-weight:700">📅 Calendario</button>'; }
function calDemo(){ const h=fechaISOLocal(new Date()); return [{id:'d1',fecha:h,titulo:'Examen Tema 3',nota:'Aula 2 · 9:00',hecho:false},{id:'d2',fecha:h,titulo:'Entregar redacción',hecho:true}]; }
async function openCalendario(mode, area){
  window._cal.mode=mode||'prof'; if(area) window._cal.area=area;
  if(!window._cal.ref) window._cal.ref=new Date();
  window._cal.sel=fechaISOLocal(new Date());
  await calCargar();
}
async function calCargar(){
  const ref=window._cal.ref, y=ref.getFullYear(), m=ref.getMonth();
  const desde=fechaISOLocal(new Date(y,m,1)), hasta=fechaISOLocal(new Date(y,m+1,0));
  let rows=[];
  try{
...
function calMover(n){ const r=window._cal.ref; window._cal.ref=new Date(r.getFullYear(), r.getMonth()+n, 1); calCargar(); }
function calSel(iso){ window._cal.sel=iso; calRender(); }
function calAdmSetArea(a){ window._cal.area=a; calCargar(); }
function calRender(){
  ...
  let sd=new Date(y,m,1).getDay(); sd=sd===0?7:sd;
  const dim=new Date(y,m+1,0).getDate(), hoy=fechaISOLocal(new Date());
  const btnNav='background:#fff;border:1px solid var(--line);border-radius:10px;width:38px;height:38px;font-size:1.2rem;cursor:pointer;color:var(--navy)';
  const h=['<button class="backbtn" onclick="'+back+'">← '+(mode==='alum'?'Volver':'Panel')+'</button>'];
  h.push('<h1 style="font-size:1.2rem;font-weight:800;color:var(--navy);margin:6px 0 10px">📅 Calendario'+(mode==='alum'?' de clase':'')+'</h1>');
  if(mode==='adm') h.push('<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px">'+CAL_AREAS.map(a=>'<button onclick="calAdmSetArea(\''+a[0]+'\')" style="font-size:.72rem;padding:5px 10px;border-radius:14px;cursor:pointer;border:1.5px solid '+(window._cal.area===a[0]?'var(--honey)':'var(--line)')+';background:'+(window._cal.area===a[0]?'var(--honey-tint)':'#fff')+';color:var(--navy);font-weight:700">'+a[1]+'</button>').join('')+'</div>');
  h.push('<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px"><button onclick="calMover(-1)" style="'+btnNav+'">‹</button><b style="font-size:1rem;color:var(--navy);text-transform:capitalize">'+calMesNombre(ref)+'</b><button onclick="calMover(1)" style="'+btnNav+'">›</button></div>');
  h.push('<div style="display:grid;grid-template-columns:repeat(7,1fr);gap:3px;margin-bottom:3px">'+CAL_DOW_H.map(d=>'<div style="text-align:center;font-size:.6rem;font-weight:800;color:var(--ink-soft)">'+d+'</div>').join('')+'</div>');
  ...
  const add=$('cal-add'); if(add) add.onclick=calAdd;
}
async function calAdd(){
  const tit=($('cal-tit')&&$('cal-tit').value||'').trim(); if(!tit){ appAlert('Escribe un título.'); return; }
  const nota=($('cal-nota')&&$('cal-nota').value||'').trim();
...
async function calHecho(id, v){
  try{ await call('/rest/v1/rpc/'+(window._cal.mode==='adm'?'cal_adm_hecho':'cal_prof_hecho'),{method:'POST',body:{p_id:id,p_hecho:v}}); await calCargar(); }
  catch(e){ appAlert('No se pudo: '+(e.message||'')); }
}
...
async function caAlumEnviar(){
  const t=$('sc-txt'); const txt=(t&&t.value||'').trim(); if(!txt) return;
  const b=$('sc-send'); if(b){ b.disabled=true; b.innerHTML='<span class="spin"></span>'; }
  try{ await call('/rest/v1/rpc/ca_alum_enviar',{method:'POST',body:{p_texto:txt}}); await caAlumChat(); }
  catch(e){ if(b){ b.disabled=false; b.textContent='Enviar'; } appAlert('No se pudo: '+(e.message||'')); }
}
// ===== fin chat profesor-alumno =====
// ===== Campanitas del alumno (gestionadas por el profe) =====
let _avAlTarget='';
async function openAvAlumnado(okMsg){
  showView('teacher'); window.scrollTo(0,0);
...
function avAlRender(lista, alumnos, okMsg, demo){
  ...
  if(!(lista||[]).length) h.push('<p style="font-size:.85rem;color:var(--ink-soft);text-align:center;margin:8px 0">Sin avisos enviados.</p>');
  else lista.forEach(m=>{
    h.push('<div class="t-card" style="padding:11px 13px"><div style="font-size:.9rem;color:var(--navy)">'+escHtml(m.texto)+'</div><div style="font-size:.72rem;color:var(--ink-soft);margin-top:3px">'+(m.alumno_id?('Para: '+escHtml(m.nombre||'un alumno')):'Toda la clase')+'</div>');
    if(!demo) h.push('<button onclick="avAlBorrar(\''+m.id+'\')" style="margin-top:8px;font-size:.72rem;padding:5px 11px;border-radius:14px;cursor:pointer;border:1.5px solid #f3c4c4;background:#fdeaea">🗑 Quitar</button>');
    h.push('</div>');
  });
  if(demo) h.push('<p style="font-size:.72rem;color:var(--ink-soft);text-align:center;margin-top:10px">Avisos de demostración.</p>');
  $('teacher').innerHTML=h.join('');
  const b=$('avl-add'); if(b) b.onclick=avAlCrear;
}
...
async function avAlBorrar(id){
  if(!await appConfirm('¿Quitar este aviso?')) return;
  try{ await call('/rest/v1/rpc/av_al_prof_borrar',{method:'POST',body:{p_id:id}}); await openAvAlumnado('Aviso quitado.'); }
  catch(e){ appAlert('No se pudo: '+(e.message||'')); }
}
...
function openSoporteProfe(){
  showView('teacher'); window.scrollTo(0,0);
  const h=['<button class="backbtn" onclick="pintarTeacher()">← Panel</button>'];
  h.push('<h1 style="font-size:1.25rem;font-weight:800;letter-spacing:-.4px;margin:6px 0 4px;color:var(--navy)">💬 Chat y reporte</h1>');
  h.push('<p style="font-size:.8rem;color:var(--ink-soft);margin-bottom:14px">Elige qué quieres abrir.</p>');
  ...
  h.push('<button onclick="openReportes()" class="t-card" style="width:100%;text-align:left;cursor:pointer;font:inherit;display:block;margin-top:10px"><div style="display:flex;justify-content:space-between;align-items:center;gap:8px"><b style="font-size:.95rem;color:var(--navy)">⚠️ Reportes de preguntas</b><span id="reportes-menu-badge"></span></div><div style="font-size:.78rem;color:var(--ink-soft);margin-top:2px">Preguntas que reporta el alumnado</div></button>');
  $('teacher').innerHTML=h.join('');
  if(!window._demoMode){
    ...
    call('/rest/v1/rpc/reportes_prof_no_leidos',{method:'POST',body:{}}).then(n=>{ const el=$('reportes-menu-badge'); if(el && +n>0){ el.className='tile-badge'; el.style.position='static'; el.style.marginLeft='6px'; el.textContent=String(n); } }).catch(()=>{});
  }
}
...
async function openPublicar(okMsg){
  ...
  getModulos().forEach(m=>{
    ...
    m.unidades.forEach(uid=>{
      ...
      any=true;
      // Título de la unidad = desplegable naranja (colapsado por defecto)
      const exs=(examsByUnit[uid]||[]).slice().sort(_cmpEx);
      const mats=(temarioByUnit[uid]||[]);
      const bodyId='unit-body-'+uid;
      h.push(`<button class="btn btn-honey unit-toggle" data-target="${bodyId}" style="width:100%;justify-content:space-between;margin:6px 0 8px"><span style="text-align:left">${escHtml(codigo)}${titulo?' · '+escHtml(titulo):''}</span><span style="font-weight:800">${exs.length?exs.length+' ':''}▾</span></button>`);
      h.push(`<div id="${bodyId}" class="hidden">`);
  ...
  $('teacher').querySelectorAll('.unit-toggle').forEach(b=> b.onclick=()=>{ const l=$(b.dataset.target); if(l){ const abre=l.classList.contains('hidden'); l.classList.toggle('hidden'); const cap=b.querySelector('span:last-child'); if(cap){ const n=cap.textContent.replace(/[^0-9]/g,''); cap.textContent=(n?n+' ':'')+(abre?'▴':'▾'); } } });
}
...
function renderExamMgmt(okMsg,errMsg){
  ...
  if(builder.kind==='redaccion'){
    ...
    h.push(listaProfHtml(units,'redaccion'));
    h.push('</div></div>');
    $('teacher').innerHTML=h.join('');
...
> Explicación</code>
    ...
    h.push('</div><div class="mgmt-right">');
    h.push(`<h2 style="font-size:.78rem;font-weight:700;color:var(--ink-soft);text-transform:uppercase;letter-spacing:1px;margin:18px 2px 12px">Exámenes creados por el profesorado</h2>`);
    h.push(listaProfHtml(units,'importar'));
    h.push('</div></div>');
    $('teacher').innerHTML=h.join('');
    const us=$('ce-unidad'); if(us) us.addEventListener('change',onUnidadChange);
...
h.push(`<label>Tema</label><select id="ce-tema">${tOpts}</select><button class="btn btn-primary" id="ce-btn" style="margin-top:16px">Crear examen</button>`);
  ...
  h.push('</div><div class="mgmt-right">');
  h.push(`<h2 style="font-size:.78rem;font-weight:700;color:var(--ink-soft);text-transform:uppercase;letter-spacing:1px;margin:18px 2px 12px">Exámenes creados por el profesorado</h2>`);
  h.push(listaProfHtml(units,'test'));
  h.push('</div></div>');
  $('teacher').innerHTML=h.join('');
  const us=$('ce-unidad'); if(us) us.addEventListener('change',onUnidadChange);
  $('kd-test').onclick=()=>setKind('test'); $('kd-red').onclick=()=>setKind('redaccion');
...
// ---- Descargar un examen en PDF para hacerlo en papel ----
async function pdfExamenPapel(examId){
  if(!window.jspdf){ appAlert('No se pudo cargar el generador de PDF. Comprueba tu conexión.'); return; }
  let ex=null;
  Object.values(examsByUnit).forEach(arr=>arr.forEach(e=>{ if(e.id===examId) ex=e; }));
  if(!ex){ appAlert('No encuentro ese examen.'); return; }
  let qs=[];
  try{ qs=await call('/rest/v1/rpc/obtener_examen_profesor',{method:'POST',body:{p_examen_id:examId}})||[]; }
  ...
  const { jsPDF }=window.jspdf;
  const doc=new jsPDF({unit:'mm',format:'a4'});
  const PW=210,PH=297,M=16, ANCHO=PW-M*2;
  const MUTED=[96,84,70],DARK=[30,26,16],LIGHT=[224,224,216];
  const HONEY=(window._activeCertId==='__aula_abierta'?[0,119,182]:[180,89,5]);
  const u=unidadesById[ex.unidad];
  const sub=(u?((u.codigo||'')+(u.titulo?' · '+tituloMateria(u):'')):(ex.unidad||''));
  ...
  function cabecera(nueva){
    ...
    if(pagina===1){
      ...
      doc.setDrawColor(LIGHT[0],LIGHT[1],LIGHT[2]);doc.setLineWidth(0.3);
      doc.line(M+34,y+1,M+118,y+1);
      doc.text('Fecha:',M+124,y);
      doc.line(M+139,y+1,PW-M,y+1);
      y+=11;
    }else{
      doc.setFont('helvetica','normal');doc.setFontSize(8.5);doc.setTextColor(MUTED[0],MUTED[1],MUTED[2]);
      doc.text(pdfSafe(ex.titulo||'Examen'),M,y);
      doc.setDrawColor(LIGHT[0],LIGHT[1],LIGHT[2]);doc.setLineWidth(0.3);doc.line(M,y+2,PW-M,y+2);
      y+=9;
    }
  }
  ...
  for(let i=0;i<qs.length;i++){
    ...
    const ops=Array.isArray(q.opciones)?q.opciones:[];
    let altoOps=0;
    if(!esRed) ops.forEach(o=>{ altoOps+=doc.splitTextToSize(pdfSafe(o),ANCHO-16).length*5+1.5; });
    hueco(enun.length*5+altoOps+(esRed?26:10));
    ...
    if(esRed){
      ...
      ops.forEach((o,oi)=>{
        const li=doc.splitTextToSize(pdfSafe(o),ANCHO-16);
        if(y+li.length*5>PH-16) cabecera(true);
        doc.setDrawColor(150,150,150);doc.setLineWidth(0.3);
        doc.rect(M+8,y-3.2,3.6,3.6);
        doc.setTextColor(MUTED[0],MUTED[1],MUTED[2]);
        doc.text((LET[oi]||String(oi+1))+')',M+14,y);
        doc.setTextColor(DARK[0],DARK[1],DARK[2]);
        li.forEach((l,k)=>{ doc.text(l,M+21,y+k*5); });
        y+=li.length*5+1.5;
      });
      y+=4;
    }
  ...
  if(conSol){
    ...
    doc.text('Hoja de soluciones',M,y); y+=3.5;
    doc.setDrawColor(HONEY[0],HONEY[1],HONEY[2]);doc.setLineWidth(0.6);doc.line(M,y,PW-M,y); y+=8;
    doc.setFontSize(9.5);
    qs.forEach((q,i)=>{
      if(y>PH-20){ doc.addPage(); pagina++; y=M; }
      const ops=Array.isArray(q.opciones)?q.opciones:[];
      const ci=q.respuesta_correcta;
      doc.setFont('helvetica','bold');doc.setTextColor(DARK[0],DARK[1],DARK[2]);
      doc.text(String(i+1)+'.  '+(LET[ci]||'?'),M,y);
      doc.setFont('helvetica','normal');doc.setTextColor(MUTED[0],MUTED[1],MUTED[2]);
      const t=doc.splitTextToSize(pdfSafe(ops[ci]||''),ANCHO-22);
      t.forEach((l,k)=>{ doc.text(l,M+20,y+k*5); });
      y+=Math.max(5,t.length*5)+1;
      if(q.explicacion){
        const e2=doc.splitTextToSize(pdfSafe(q.explicacion),ANCHO-22);
        if(y+e2.length*4.5>PH-16){ doc.addPage(); pagina++; y=M; }
        doc.setFontSize(8.5);doc.setTextColor(120,120,120);
        e2.forEach((l,k)=>{ doc.text(l,M+20,y+k*4.5); });
        y+=e2.length*4.5+2; doc.setFontSize(9.5);
      }
    });
  }
  ...
  for(let i=1;i<=pages;i++){
    ...
    doc.text(pdfSafe('Aptuvia - '+sub),M,PH-8);
    doc.text('Pagina '+i+' de '+pages,PW-M,PH-8,{align:'right'});
  }
  docVer(doc,'Aptuvia_examen_'+fmtStamp(fecha)+'.pdf');
}
...
function listaProfHtml(units, kind){
  ...
  return any?list:`<div class="center-msg" style="padding:18px">Aún no has creado exámenes de esta modalidad.</div>`;
}
...
// ---- Datos agregados del centro (los usan el informe y las alertas) ----
// Se recalcula con el mismo criterio que ve el profesor, así que hay que
// apoyarse en las cachés globales: se guardan y se devuelven al terminar.
async function acRecopilarCentro(botonId){
  const profes=(window._acadProfes||[]).slice();
  if(!profes.length){ appAlert('Todavía no hay profesorado en el centro.'); return null; }
  const btn=botonId?$(botonId):null; const txtBtn=btn?btn.innerHTML:'';
  if(btn){ btn.disabled=true; btn.innerHTML='<span class="spin"></span> Recopilando…'; }
  const _bkUnid=unidadesById, _bkExam=examsByUnit, _bkRows=teacherRows, _bkRes=resumenRows;
  const datos=[];
  try{
    for(let i=0;i<profes.length;i++){
      const p=profes[i];
      if(btn) btn.innerHTML='<span class="spin"></span> '+(i+1)+' de '+profes.length;
      let d=null;
      try{ d=await call('/rest/v1/rpc/ac_panel',{method:'POST',body:{p_profesor:p.id}}); }catch(e){ d=null; }
      if(!d){ datos.push({p:p, error:true, alumnos:[]}); continue; }
      unidadesById={}; (d.unidades||[]).forEach(u=>{ unidadesById[u.id]=u; });
      examsByUnit={};
      ...
      const porAlumno={};
      rows.forEach(r=>{ const e=r.alumno_email; if(!e) return;
        (porAlumno[e]=porAlumno[e]||{email:e,nombre:r._nombre||null,rows:[]}).rows.push(r);
        if(r._nombre) porAlumno[e].nombre=r._nombre; });
      const alumnos=Object.values(porAlumno).map(a=>({
        nombre:a.nombre||a.email, email:a.email,
        mc:mediaPorUnidad(a.rows,null), mf:mediaFinalPorUnidad(a.rows,null),
        intentos:a.rows.filter(r=>!esIntentoFantasma(r)).length
      })).sort((x,y)=>String(x.nombre).localeCompare(String(y.nombre),'es'));
      let nEx=0, nPub=0;
      Object.values(examsByUnit).forEach(arr=>(arr||[]).forEach(e=>{ nEx++; if(e.publicado) nPub++; }));
      datos.push({p:p, error:false, alumnos:alumnos,
        nExamenes:nEx, nPublicados:nPub, nUnidades:Object.keys(unidadesById).length});
    }
  } finally {
    unidadesById=_bkUnid; examsByUnit=_bkExam; teacherRows=_bkRows; resumenRows=_bkRes;
    if(btn){ btn.disabled=false; btn.innerHTML=txtBtn; }
  }
  return datos;
}

// ---- Alumnado en riesgo (rol academia · Premium) ----
function acMotivoRiesgo(a){
  if(a.intentos===0) return {txt:'Sin actividad', peso:0, det:'No ha hecho ningún examen todavía.'};
  if(a.mc!=null && a.mc<3) return {txt:'Media muy baja', peso:1, det:'Media de clase '+a.mc.toFixed(1)+' / 10.'};
  if(a.mc!=null && a.mc<5) return {txt:'Por debajo del 5', peso:2, det:'Media de clase '+a.mc.toFixed(1)+' / 10.'};
  if(a.mf!=null && a.mf<5) return {txt:'Suspende la nota final', peso:3, det:'Media final '+a.mf.toFixed(1)+' / 10.'};
  return null;
}
async function acAlertasRiesgo(){
  const datos=await acRecopilarCentro('ac-riesgo');
  if(!datos) return;
  const lista=[];
  ...
  h+=`<h1 style="font-size:1.2rem;font-weight:800;letter-spacing:-.4px;margin:8px 0 2px;color:var(--navy)">Alumnado en riesgo</h1>`;
  h+=`<p style="font-size:.78rem;color:var(--ink-soft);margin:0 2px 14px">${lista.length} de ${totAl} alumno${totAl===1?'':'s'} necesita${lista.length===1?'':'n'} atención. Se marca a quien no ha empezado y a quien no llega al 5.</p>`;
  if(!lista.length){
    h+=`<div class="soon-screen"><div class="soon-emoji">✅</div><div class="soon-title">Nadie en riesgo</div>
      <div class="soon-text">Todo el alumnado con actividad supera el 5 de media.</div></div>`;
  }else{
    lista.forEach(x=>{
      ...
      h+=`<button class="al-row" data-acprof2="${escAttr(x.p.id)}" style="width:100%;text-align:left">
        <span class="al-info"><b>${escHtml(x.a.nombre)}</b>
          ...
          <span>${escHtml(x.p.nombre||x.p.email||'')}${x.p.cert_codigo?' · '+escHtml(x.p.cert_codigo):''}</span></span>
        <span class="arrow">›</span>
      </button>`;
    });
  }
  ...
  rows.forEach(r=>{
    ...
    h+=`<div class="t-card" style="margin-bottom:9px">
      ...
      <div style="height:7px;background:#eef0f4;border-radius:99px;margin:7px 0 8px;overflow:hidden">
        <div style="height:100%;width:${ancho}%;background:${color};border-radius:99px"></div>
      </div>
      <div style="font-size:.76rem;color:var(--ink-soft);line-height:1.7">
...
async function acFacturacion(){
  ...
  $('teacher').innerHTML='<button class="backbtn" onclick="openAcademiaCentro()">← Centro</button><div class="loader"><span class="spin"></span></div>';
  let fs=[];
  try{ fs=await call('/rest/v1/rpc/ac_facturas',{method:'POST',body:window._acadVista?{p_academia:window._acadVista}:{}})||[]; }
  catch(err){
    $('teacher').innerHTML=`<button class="backbtn" onclick="openAcademiaCentro()">← Centro</button>
      <div class="center-msg">No se pudo cargar la facturación.<br><small>${escHtml(err.message||'')}</small></div>`;
    return;
  }
  window._acFacturas=fs;
  const sum=arr=>arr.reduce((t,f)=>t+(Number(f.total)||0),0);
  let h=`<button class="backbtn" onclick="openAcademiaCentro()">← Centro</button>`;
  h+=`<h1 style="font-size:1.2rem;font-weight:800;letter-spacing:-.4px;margin:8px 0 2px;color:var(--navy)">Facturación</h1>`;
  h+=`<p style="font-size:.78rem;color:var(--ink-soft);margin:0 2px 14px">Tus facturas. Toca cualquiera para abrirla en PDF y descargarla.</p>`;
  if(!fs.length){
    h+=`<div class="soon-screen"><div class="soon-emoji">🧾</div><div class="soon-title">Todavía no hay facturas</div>
      <div class="soon-text">Cuando se emita la primera, aparecerá aquí.</div></div>`;
  }else{
    h+=`<div class="t-card" style="margin-bottom:14px;font-size:.85rem;line-height:1.8">
      Facturas: <b>${fs.length}</b><br>
      Total facturado: <b>${gxEur(sum(fs))}</b>
    </div>`;
    fs.forEach(f=>{
      h+=`<button class="al-row" data-acfac="${escAttr(f.id)}" style="width:100%;text-align:left">
        <span class="al-info"><b>Factura ${escHtml(f.numero||'')}</b>
          <span>${escHtml(fmtFecha(f.fecha))}</span>
          <span style="color:${f.pagada?'#1c7a44':'#b4232a'};font-weight:700">${gxEur(Number(f.total)||0)} · ${f.pagada?'Pagada':'Pendiente'}</span></span>
        <span class="arrow">›</span>
      </button>`;
    });
  }
  $('teacher').innerHTML=h;
  $('teacher').querySelectorAll('[data-acfac]').forEach(b=> b.onclick=()=>acVerFactura(b.dataset.acfac));
}
function acVerFactura(id){
  const fc=(window._acFacturas||[]).find(x=>String(x.id)===String(id));
  if(!fc){ appAlert('No se encontró la factura.'); return; }
  const lin=fc.lineas||{};
  factRenderPDF({
    numero:fc.numero, fecha:fc.fecha,
    nombreCliente:(fc.cliente&&fc.cliente.razon_social)||window._acadNombre||'cliente',
    d:fc.cliente||{}, rec:lin.rec||[], uni:lin.uni||[],
    descuento:Number(fc.descuento_pct)||0, iva:Number(fc.iva_pct)||21,
    presuNumero:fc.presupuesto_numero||''
  });
...
// ---- Actividad del profesorado (rol academia · Premium) ----
// Seguimiento del servicio contratado: qué contenido hay montado y cuánto se
// está usando. No es una herramienta de control laboral y no se presenta así.
async function acActividadProfesorado(){
  const datos=await acRecopilarCentro('ac-actividad');
  if(!datos) return;
  const filas=datos.map(d=>{
    const al=d.alumnos||[];
    const act=al.filter(a=>a.intentos>0).length;
    const intentos=al.reduce((n,a)=>n+a.intentos,0);
    const conMedia=al.filter(a=>a.mc!=null);
    const media=conMedia.length? conMedia.reduce((n,a)=>n+a.mc,0)/conMedia.length : null;
    return {p:d.p, error:d.error, nEx:d.nExamenes||0, nPub:d.nPublicados||0, nUn:d.nUnidades||0,
      nAl:al.length, act:act, intentos:intentos, media:media};
  });
  showView('teacher'); window.scrollTo(0,0);
  let h=`<button class="backbtn" onclick="openAcademiaCentro()">← Centro</button>`;
  h+=`<h1 style="font-size:1.2rem;font-weight:800;letter-spacing:-.4px;margin:8px 0 2px;color:var(--navy)">Actividad del profesorado</h1>`;
  h+=`<p style="font-size:.78rem;color:var(--ink-soft);margin:0 2px 14px">Contenido montado en la plataforma y uso que está teniendo. Sirve para ver dónde hace falta apoyo, no para comparar a unos con otros.</p>`;
  filas.forEach(f=>{
    if(f.error){
      h+=`<div class="t-card" style="margin-bottom:10px"><b style="font-size:.92rem">${escHtml(f.p.nombre||f.p.email||'Profesor')}</b>
        <div style="font-size:.78rem;color:var(--ink-soft);margin-top:4px">No se pudieron cargar sus datos.</div></div>`;
      return;
    }
...
// ---- Informe del centro en PDF (rol academia · Premium) ----
async function pdfInformeCentro(){
  if(!window.jspdf){ appAlert('No se pudo cargar el generador de PDF. Comprueba tu conexión.'); return; }
  const datos=await acRecopilarCentro('ac-informe');
  if(!datos) return;

  const { jsPDF }=window.jspdf;
  const doc=new jsPDF({unit:'mm',format:'a4'});
  const PW=210,PH=297,M=16, ANCHO=PW-M*2;
  const MUTED=[96,84,70],DARK=[30,26,16],LIGHT=[224,224,216],NAVY=[46,49,99];
  const HONEY=(window._activeCertId==='__aula_abierta'?[0,119,182]:[180,89,5]);
  const GREEN=[21,128,61],RED=[192,57,43];
  const fecha=new Date();
  let y=M;
  const cAl=M, cMc=M+92, cMf=M+118, cIn=M+144, cEs=M+164;

  function cabeceraPagina(primera){
    if(!primera){ doc.addPage(); y=M; }
    y=pdfCabeceraMarca(doc,y,{hueco:5});
    if(primera){
      doc.setFont('helvetica','bold');doc.setFontSize(13);doc.setTextColor(DARK[0],DARK[1],DARK[2]);
      doc.text('Informe del centro',M,y); y+=5;
      doc.setFont('helvetica','normal');doc.setFontSize(9);doc.setTextColor(MUTED[0],MUTED[1],MUTED[2]);
      doc.text(pdfSafe(fmtFechaES(fecha)),M,y); y+=3.5;
      doc.setDrawColor(HONEY[0],HONEY[1],HONEY[2]);doc.setLineWidth(0.6);doc.line(M,y,PW-M,y); y+=8;
    }else{
      doc.setDrawColor(LIGHT[0],LIGHT[1],LIGHT[2]);doc.setLineWidth(0.3);doc.line(M,y-2,PW-M,y-2); y+=4;
    }
  }
  function ensure(sp){ if(y+sp>PH-16) cabeceraPagina(false); }
  function colHeaders(){
    doc.setFont('helvetica','bold');doc.setFontSize(8);doc.setTextColor(MUTED[0],MUTED[1],MUTED[2]);
    doc.text('Alumno',cAl,y);doc.text('Clase',cMc,y);doc.text('Final',cMf,y);doc.text('Intentos',cIn,y);doc.text('Estado',cEs,y);
  ...
  // Resumen global
  let totAl=0, totAct=0, totApr=0, sumaMc=0, nMc=0, enRiesgo=0;
  datos.forEach(d=>{
    totAl+=d.alumnos.length;
    d.alumnos.forEach(a=>{
      if(a.intentos>0) totAct++;
      if(a.mc!=null){ sumaMc+=a.mc; nMc++; if(a.mc>=5) totApr++; else enRiesgo++; }
      else enRiesgo++;
    });
  });
  const mediaCentro=nMc?(sumaMc/nMc):null;
  ...
  [
    'Profesorado: '+datos.length,
    'Alumnado: '+totAl+'   ·   con actividad: '+totAct,
    'Media del centro: '+(mediaCentro!=null?mediaCentro.toFixed(1)+' / 10':'sin datos'),
    'Aprobados (media 5 o mas): '+totApr+'   ·   por debajo o sin actividad: '+enRiesgo
  ].forEach(l=>{ doc.text(pdfSafe(l),M,y); y+=5; });
  y+=5;

  datos.forEach(d=>{
    ensure(26);
    doc.setDrawColor(HONEY[0],HONEY[1],HONEY[2]);doc.setLineWidth(0.5);doc.line(M,y-3,PW-M,y-3);
    doc.setFont('helvetica','bold');doc.setFontSize(10.5);doc.setTextColor(DARK[0],DARK[1],DARK[2]);
    doc.text(pdfSafe(d.p.nombre||d.p.email||'Profesor'),M,y); y+=4.5;
    doc.setFont('helvetica','normal');doc.setFontSize(8.5);doc.setTextColor(MUTED[0],MUTED[1],MUTED[2]);
    doc.text(pdfSafe([d.p.cert_codigo,d.p.cert_nombre].filter(Boolean).join(' · ')||'Sin certificado'),M,y); y+=4.5;
    if(!d.error){
      doc.text(pdfSafe((d.nExamenes||0)+' examenes montados   ·   '+(d.nPublicados||0)+' visibles   ·   '+(d.nUnidades||0)+' unidades'),M,y); y+=6;
    } else { y+=1.5; }
    if(d.error){ doc.setFontSize(9);doc.text('No se pudieron cargar sus datos.',M,y); y+=8; return; }
    if(!d.alumnos.length){ doc.setFontSize(9);doc.text('Sin alumnado con actividad.',M,y); y+=8; return; }
    colHeaders();
    d.alumnos.forEach(a=>{
      ensure(7);
      const mc=(a.mc!=null)?a.mc.toFixed(1):'-';
      const mf=(a.mf!=null)?a.mf.toFixed(1):'-';
      const riesgo=(a.intentos===0)||(a.mc!=null&&a.mc<5);
  ...
  for(let i=1;i<=pages;i++){
    doc.setPage(i);doc.setFont('helvetica','normal');doc.setFontSize(7.5);doc.setTextColor(150,150,150);
    doc.text(pdfSafe('Aptuvia - Informe del centro'),M,PH-8);
    doc.text('Pagina '+i+' de '+pages,PW-M,PH-8,{align:'right'});
  }
...
async function openAcademiaCentro(msg, academiaId){
  ...
  grCargarResumen().then(rows=>{
    ...
    if(ref&&ref.nextSibling) cont.insertBefore(b, ref.nextSibling.nextSibling||ref.nextSibling); else cont.insertBefore(b, cont.firstChild);
  });
  ...
  rows.forEach(r=>{
    ...
    h+=`<div class="reg-row">
      <div class="reg-info">
        ...
        <div class="reg-badges" style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-top:6px">
          ${badge}${act}
...
async function pdfMiRedaccion(entrega){
  ...
  const MUTED=[96,84,70],DARK=[30,26,16],LIGHT=[224,224,216],NAVY=[46,49,99];
  const HONEY=(window._activeCertId==='__aula_abierta'?[0,119,182]:[180,89,5]);
  const u=unidadesById[current.unit];
  const sub=(u?((u.codigo||'')+(u.titulo?' · '+tituloMateria(u):'')):'');
  const fecha=new Date();
  ...
  function cabecera(nueva){
    ...
    if(pagina===1){
      y=pdfCabeceraMarca(doc,y);
      doc.setFont('helvetica','bold');doc.setFontSize(13);doc.setTextColor(DARK[0],DARK[1],DARK[2]);
      doc.splitTextToSize(pdfSafe(redCurrent.titulo||'Examen'),ANCHO).forEach(l=>{ doc.text(l,M,y); y+=6; });
      doc.setFont('helvetica','normal');doc.setFontSize(9);doc.setTextColor(MUTED[0],MUTED[1],MUTED[2]);
      doc.text(pdfSafe('Alumno: '+(userName||'')+(sub?'   ·   '+sub:'')),M,y); y+=4.5;
      ...
      if(nota!=null){
        ...
        doc.text('Nota: '+nota+' / 10',M,y); y+=9;
      }
    }else{
      doc.setFont('helvetica','normal');doc.setFontSize(8.5);doc.setTextColor(MUTED[0],MUTED[1],MUTED[2]);
      doc.text(pdfSafe(redCurrent.titulo||'Examen'),M,y);
      doc.setDrawColor(LIGHT[0],LIGHT[1],LIGHT[2]);doc.setLineWidth(0.3);doc.line(M,y+2,PW-M,y+2);
      y+=9;
    }
  }
  function bloque(rotulo, texto, cursiva){
    if(!String(texto||'').trim()) return;
    const lineas=doc.splitTextToSize(pdfSafe(String(texto).trim()),ANCHO-7);
    if(y+10>PH-16) cabecera(true);
    doc.setFont('helvetica','bold');doc.setFontSize(7.6);doc.setTextColor(MUTED[0],MUTED[1],MUTED[2]);
    doc.text(rotulo.toUpperCase(),M+7,y); y+=4.2;
    doc.setFont('helvetica',cursiva?'italic':'normal');doc.setFontSize(9.5);
    doc.setTextColor(cursiva?NAVY[0]:DARK[0],cursiva?NAVY[1]:DARK[1],cursiva?NAVY[2]:DARK[2]);
    lineas.forEach(l=>{ if(y>PH-18) cabecera(true); doc.text(l,M+7,y); y+=5; });
    y+=3;
  }
  ...
  for(let i=0;i<_preg.length;i++){
    ...
    doc.text('Pregunta '+(i+1),M,y); y+=5.5;
  ...
  for(let i=1;i<=pages;i++){
    doc.setPage(i);doc.setFont('helvetica','normal');doc.setFontSize(7.5);doc.setTextColor(150,150,150);
    doc.text(pdfSafe('Aptuvia - '+(userName||'')),M,PH-8);
    doc.text('Pagina '+i+' de '+pages,PW-M,PH-8,{align:'right'});
  }
  docVer(doc,'Aptuvia_examen_corregido_'+fmtStamp(fecha)+'.pdf');
}
...
function renderRedaccion(entrega){
  ...
  if($('red-pdf')) $('red-pdf').onclick=()=>pdfMiRedaccion(entrega);
}
...
function generarPDF(){
  ...
  // Cabecera
  y=pdfCabeceraMarca(doc,y,{margen:M,hueco:5});
  doc.setFont('helvetica','normal');doc.setFontSize(9.5);doc.setTextColor(MUTED[0],MUTED[1],MUTED[2]);
  doc.text('Justificante de autoevaluación',M,y); y+=4.5;
  const _dat=pdfLineaDatos('');
  if(_dat){ doc.setFontSize(9); doc.text(pdfTxt(_dat),M,y); y+=4.5; }
  doc.setDrawColor(HONEY[0],HONEY[1],HONEY[2]);doc.setLineWidth(0.6);doc.line(M,y,PW-M,y); y+=8;
...
// ===== DEMO · Vista de grupo (cuenta de dirección) =====
...
const DEMO_GR_PROFES = {
  901:[
    {id:'dgp-901-1', nombre:'Nuria Salas (demo)',   email:'nuria@demo.local',   cert_codigo:'ADGG0508', cert_nombre:'Operaciones de grabación y tratamiento de datos', n_alumnos:10, academia_nombre:'Academia Demo Norte'},
    {id:'dgp-901-2', nombre:'Iván Prieto (demo)',   email:'ivan@demo.local',    cert_codigo:'COMT0411', cert_nombre:'Gestión comercial de ventas',                  n_alumnos:10, academia_nombre:'Academia Demo Norte'},
    {id:'dgp-901-3', nombre:'Sara Núñez (demo)',    email:'sara@demo.local',    cert_codigo:'SSCS0208', cert_nombre:'Atención sociosanitaria a domicilio',           n_alumnos:10, academia_nombre:'Academia Demo Norte'}
  ],
  902:[
    {id:'dgp-902-1', nombre:'Pablo Gil (demo)',     email:'pablo@demo.local',   cert_codigo:'IFCT0209', cert_nombre:'Sistemas microinformáticos',                    n_alumnos:10, academia_nombre:'Academia Demo Sur'},
    {id:'dgp-902-2', nombre:'Laura Vega (demo)',    email:'laura@demo.local',   cert_codigo:'HOTR0208', cert_nombre:'Operaciones básicas de cocina',                  n_alumnos:10, academia_nombre:'Academia Demo Sur'},
    {id:'dgp-902-3', nombre:'Diego Mora (demo)',    email:'diego@demo.local',   cert_codigo:'ADGD0208', cert_nombre:'Gestión integrada de recursos humanos',          n_alumnos:10, academia_nombre:'Academia Demo Sur'}
  ]
};
...
function demoGrAlumnos(seed, n){
  ...
  for(let i=0;i<n;i++){
    ...
    const r=(x%1000)/1000;
    const nombre=DEMO_GR_NOMBRES[(seed+i)%DEMO_GR_NOMBRES.length]+' '+String.fromCharCode(65+((seed+i)%26))+'. (demo)';
    let intentos, mc, mf;
    if(r<0.15){ intentos=0; mc=null; mf=null; }                 // sin actividad
    else if(r<0.35){ intentos=3+(i%3); mc=3.2+r*1.6; mf=2.8+r*1.7; }  // en riesgo
    else { intentos=3+(i%5); mc=5.5+r*3.6; mf=5.0+r*3.8; }       // bien
    out.push({ nombre, email:'alumno'+seed+'_'+i+'@demo.local',
      mc:mc==null?null:Math.min(10,+mc.toFixed(1)),
      mf:mf==null?null:Math.min(10,+mf.toFixed(1)),
      intentos });
  }
  return out;
}
...
function demoResponder(path, opts){
  ...
  if(p.indexOf('/rest/v1/academia')===0){
    if(window._demoAcadVista===902) return [{nombre:'Academia Demo Sur', nombre_aula_abierta:'Aula Abierta Demo'}];
    if(window._demoPerfil&&window._demoPerfil.rol==='academia') return [{nombre:'Academia Demo Norte', nombre_aula_abierta:'Aula Abierta Demo'}];
    return [{nombre:'Academia Demo', nombre_aula_abierta:'Aula Abierta Demo'}];
  }
  ...
  if(p.indexOf('/rest/v1/rpc/ac_panel')===0){
    const pid=body.p_profesor||'';
    const seed=(pid.charCodeAt(pid.length-1)||65);
    const als=demoGrAlumnos(seed,10);
    // ac_panel real devuelve unidades/examenes/resultados. Aquí basta con
    // resultados en el formato "nombre|email" que espera acVerProfesor.
    const resultados=[];
    als.forEach(a=>{
      const nInt=a.intentos; 
      for(let k=0;k<nInt;k++){
        const total=10, correctas=Math.round(((a.mc||5)/10)*total);
        resultados.push({alumno:a.nombre+'|'+a.email, unidad:'demo-gu1', examen:'Examen '+(k+1), correctas, total});
      }
      if(a.mf!=null){ const total=10, correctas=Math.round((a.mf/10)*total);
        resultados.push({alumno:a.nombre+'|'+a.email, unidad:'demo-gu1', examen:'Examen final', correctas, total}); }
    });
    return { unidades:[{id:'demo-gu1', codigo:'MF-DEMO', titulo:'Unidad demo', modulo:'Demo', orden:1, estado:'activo', ver_megatest:true, ver_falladas:true, certificado_id:'demo', academia_id:901}],
      examenes:[{id:'demo-gx-final', unidad:'demo-gu1', titulo:'Examen final', tipo:'test', cuenta_final:true, publicado:true, academia_id:901}],
      resultados };
  }
  if(p.indexOf('/rest/v1/rpc/ac_facturas')===0){
    return [
      {id:'df1', numero:'2026-014', fecha:'2026-07-01', total:363, subtotal:300, descuento_pct:0, iva_pct:21, pagada:true,  enviada:true, cliente:{razon_social:'Academia Demo Norte'}, lineas:{rec:[{nombre:'Acceso mensual · 3 profesores',precio:100,cant:3}],uni:[]}, presupuesto_numero:'P-2026-011'},
      {id:'df2', numero:'2026-009', fecha:'2026-06-01', total:363, subtotal:300, descuento_pct:0, iva_pct:21, pagada:true,  enviada:true, cliente:{razon_social:'Academia Demo Norte'}, lineas:{rec:[{nombre:'Acceso mensual · 3 profesores',precio:100,cant:3}],uni:[]}, presupuesto_numero:''},
      {id:'df3', numero:'2026-021', fecha:'2026-07-15', total:181.5,subtotal:150, descuento_pct:0, iva_pct:21, pagada:false, enviada:true, cliente:{razon_social:'Academia Demo Norte'}, lineas:{rec:[],uni:[{nombre:'Activación certificado ADGG0508',precio:150,cant:1}]}, presupuesto_numero:''}
    ];
  }
...
function demoPortal(){
  const h=`<div class="portal-card demo2" style="text-align:left">
    ...
    <div class="demo2-btns">
      ...
      <button class="btn btn-portal" onclick="demoEntrar('grupo')" style="background:linear-gradient(to right,#efeafd,#c3b2ea);border:1.5px solid #a78bda;color:var(--navy);font-weight:800;margin-bottom:8px">Dirección de grupo · Premium</button>
...
async function demoEntrar(perfil){
  ...
  if(perfil==='prof'){                 // Profesor · Aptuvia (EV)
    ...
    window._activeCertId='__aula_abierta';
  }else if(perfil==='grupo'){          // Dirección de grupo · Premium
    window._demoPerfil={id:'demo-dir', nombre:'Dirección Grupo Demo', rol:'academia', academia_id:901, profesor_id:null, grupo_id:1};
    window._acadModo=true; window._acadVista=null; window._demoAcadVista=901;
    window._activeCertId=DEMO_PORTAL_ID;
